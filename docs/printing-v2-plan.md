...
## 12. Zoho Inventory — quantity adjustments per job (built 2026-09-23)

Supersedes the "Inventory — still deferred" rows above. Printing now moves Zoho Inventory, as
**quantity adjustments, one per job per leg**, posted the moment the store person presses the
button — the same "document per event" shape as `postTransferOrders`, not the batched queue.

| Leg | When | Item | Qty | Reference |
|---|---|---|---|---|
| OUT | `sendToPrint` | source (plain) SKU | **−Metres_Sent** | `PJ-00001-OUT` |
| IN | `receiveFromPrint` | printed SKU | **+Metres_Returned** | `PJ-00001-IN` |
| RET | `cancelPrintJob` | source (plain) SKU | **+Metres_Sent** | `PJ-00001-RET` |

- **Location `3955559000000032097`** — the one `postTransferOrders` moves cloth *out of*. Printing
  is before issue, so cloth leaves and returns there. (CLAUDE.md's location table calls this id
  *Head Office* and gives Main Warehouse as `…212001`; the code everywhere treats `…032097` as the
  store. Confirmed with the user for this build.)
- **One account both sides** — `3955559000000032057`, the consumption account. The conversion nets
  and what remains is the value lost at the printer. The loss is never posted on its own: OUT took
  every metre that left, IN adds back only what returned.
- **Reason `Production Cycle`** — the one already proven on this org by `postInventoryQueue`. A
  dedicated reason (e.g. "Fabric Printing") needs adding in Inventory first; it is one constant.
- **`PJ-NNNNN`** — `Print_Job.Job_No`, generated by `sendToPrint` the way the SIV is (sorted on ID,
  largest parsed number wins). It is the reference on every adjustment.

### `postPrintAdjustments(jobIdTxt, dryRun)`

The only thing that talks to Inventory for printing. The three writers call it inline via
`thisapp`, in their own try/catch, **after** their Creator writes — an unreachable Inventory never
makes a completed send/receipt/cancel look failed. It reports back as `inventory` +
`inventoryError` in each writer's response, and the widget alerts when it is not `Posted`.

`jobIdTxt == ""` is a **sweep** of every job with a leg at `Pending`/`Posting` (oldest 20). Nothing
schedules it yet — Execute it by hand, or schedule a no-argument wrapper.

Leg states (`Inv_Out_Status`, `Inv_In_Status`): `Pending` → `Posting` (written **before** the
call) → `Posted`, or `Not_Needed`. A leg found at `Posting` is checked with a GET by reference
before anything is re-posted — the statement limit is not catchable, and without this a crash after
the POST double-posts. Same recovery as `Inventory_Post_Queue`.

Rules it holds, each pinned by `tools/print-inventory.test.js`:
- **IN / RET wait for OUT.** Never +printed while −plain is still owed; never give back cloth
  Inventory never took.
- **A cancel whose OUT never landed posts nothing** — both legs go `Not_Needed`.
- **Blank `Inv_Out_Status` = a job sent before this existed.** Named explicitly (by its receipt or
  cancel) it is treated as `Pending` — Creator took the plain cloth off, Inventory did not, so the
  OUT is the correction. The sweep ignores blanks: jobs already received/cancelled before the
  deploy are left for a human.
- **Unmapped SKU:** `sendToPrint` refuses the send if either SKU has no `Inventory_Item_ID`. At
  receipt/cancel it does not refuse (the cloth is physically back); the leg stays `Pending` with the
  reason in `Inv_Last_Error`.

### Creator changes (manual)

`Print_Job`, new fields — nothing mandatory:

| Field | Type | Notes |
|---|---|---|
| `Job_No` | Single Line | `PJ-00001` |
| `Inv_Out_Status` | Dropdown | `Pending`, `Posting`, `Posted`, `Not_Needed` — **no default** |
| `Inv_In_Status` | Dropdown | same four — **no default** (blank while at the printer) |
| `Inv_Out_Doc_ID` | Single Line | Inventory adjustment id |
| `Inv_In_Doc_ID` | Single Line | Inventory adjustment id |
| `Inv_Last_Error` | Multi Line | why a leg did not post |

`Print_Job_Report` must include `Job_No`, `Inv_Out_Status`, `Inv_Last_Error` (the widget reads them).

New Custom API **`postPrintAdjustments`**, two string args `jobIdTxt`, `dryRun`. The three writers'
argument lists are unchanged.

**Test order:** `postPrintAdjustments("<an At_Printer job id>","true")` → check the logged body →
send one real small job → confirm `PJ-…-OUT` in Inventory at the right location → receive it →
confirm `PJ-…-IN` → send and cancel another → confirm `-RET` nets to zero.
